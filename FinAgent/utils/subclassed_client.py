from langchain_community.vectorstores.pathway import PathwayVectorClient, _VectorStoreClient
import json
import logging
...
# to reduce dependency on processing power, we add capability to set timeout for the requests
class _VectorStoreClient_with_timeout(_VectorStoreClient):
    def __init__(
        self,
        ...
        """
        super().__init__(host=host, port=port, url=url)
...
class PathwayVectorClient_with_timeout(PathwayVectorClient):
    """
    ...
    def __init__(
        ...
        """
        self.client = _VectorStoreClient_with_timeout(host, port, url)
    ...
    def similarity_search(
        ...
        rets = self.client(query=query, k=k, metadata_filter=metadata_filter, timeout=timeout)
    ...
    def get_input_files(
        ...
        return self.client.get_input_files(metadata_filter, filepath_globpattern, timeout=timeout)
