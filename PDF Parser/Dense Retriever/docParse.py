...
class DoclingParser:
    ...
    def table_column_aggregation(self, table_html):
        ...
        largest_cells = []
        cells0 = rows[0]
        for cell in cells0:  largest_cells.append(cell.get_text())
        for row in rows[1:]:
        ...
        for tag in soup.document.children:
            if tag.name:
                if tag.name == "section_header":
                    ...
                    cur_text = ""
                    self.add_to_json_final(json_final, 'list', header, list_items=list_items)
                    list_items = []
                    header = tag.text
                    if header == 'Table of Contents': header = ""
